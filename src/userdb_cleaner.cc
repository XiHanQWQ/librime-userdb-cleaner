...
#if defined(_WIN32) || defined(_WIN64)
/**
 * 执行 WeaselDeployer 命令（无窗口模式）- 只执行 /sync
 */
bool execute_weasel_sync() {
  // 获取共享数据目录（程序目录）- 使用非安全版本
  ...
  std::string command = "\"" + deployer_path.string() + "\" /sync";
  ...
  LOG(INFO) << "WeaselDeployer sync executed successfully";
  return true;
}
#endif

/**
 * 获取同步目录 - 只使用 get_sync_dir
 */
fs::path get_sync_directory() {
  // 使用 get_sync_dir API 函数（非安全版本）
  const char* sync_dir = rime_get_api()->get_sync_dir();
  
  if (!sync_dir) {
    LOG(ERROR) << "Failed to get sync directory from API";
    return fs::path();
  }
  
  fs::path sync_path = fs::path(sync_dir);
  
  if (fs::exists(sync_path) && fs::is_directory(sync_path)) {
    LOG(INFO) << "Using sync directory: " << sync_path.string();
    return sync_path;
  }
...
int clean_userdb_folders() {
  // 使用 get_user_data_dir 获取用户数据目录（非安全版本）
  const char* user_data_dir = rime_get_api()->get_user_data_dir();
...
std::vector<fs::path> get_userdb_files() {
  ...
  if (sync_path.empty()) {
    LOG(ERROR) << "Cannot get sync directory";
    return result;
  }
...
//   // 方法1: 使用 get_sync_dir API 函数（非安全版本）
//   const char* sync_dir = rime_get_api()->get_sync_dir();
//   if (sync_dir) {
...
//   } else {
//     LOG(WARNING) << "Failed to get sync directory from API";
//   }
